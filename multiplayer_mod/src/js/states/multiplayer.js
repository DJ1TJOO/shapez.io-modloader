...
export class MultiplayerState extends shapezAPI.exports.GameState {
            ...
            <div class="mainWrapper ${showDemoBadges ? "demo" : "noDemo"}">
                ...
                <div class="mainContainer">
                    ${
                        isSupportedBrowser()
                            ? ""
                            : `<div class="browserWarning">${shapezAPI.translations.mainMenu.browserWarning}</div>`
                    }
            ...
            if (file) {
                ...
                waitNextFrame().then(() => {
                    ...
                    reader.addEventListener("load", event => {
                        ...
                        try {
                            ...
                            this.dialogs.showWarning(
                                shapezAPI.translations.dialogs.importSavegameError.title,
                                shapezAPI.translations.dialogs.importSavegameError.text + "<br><br>" + err
                            );
                            return;
                        }
                        ...
                        this.app.savegameMgr.importSavegame(realContent).then(
                            () => {
                                closeLoader();
                                this.dialogs.showWarning(
                                    shapezAPI.translations.dialogs.importSavegameSuccess.title,
                                    shapezAPI.translations.dialogs.importSavegameSuccess.text
                                );
                            ...
                            err => {
                                ...
                                this.dialogs.showWarning(
                                    shapezAPI.translations.dialogs.importSavegameError.title,
                                    shapezAPI.translations.dialogs.importSavegameError.text +
                                        ":<br><br>" +
                                        err
                                );
                            }
                    ...
                    reader.addEventListener("error", error => {
                        this.dialogs.showWarning(
                            shapezAPI.translations.dialogs.importSavegameError.title,
                            shapezAPI.translations.dialogs.importSavegameError.text + ":<br><br>" + error
                        );
                    });
    ...
    onEnter(payload) {
        ...
        if (payload.loadError) {
            this.dialogs.showWarning(
                shapezAPI.translations.dialogs.gameLoadFailure.title,
                shapezAPI.translations.dialogs.gameLoadFailure.text + "<br><br>" + payload.loadError
            );
        }
    ...
    renderMainMenu() {
        ...
        const joinButton = makeButton(
            ...
            shapezAPI.translations.multiplayer.join
        );
        ...
        const backButton = makeButton(
            ...
            shapezAPI.translations.multiplayer.back
        );
    ...
    onLanguageChooseClicked() {
        ...
        const { optionSelected } = this.dialogs.showOptionChooser(
            shapezAPI.translations.settings.labels.language.title,
            {
                ...
                if (setting.restartRequired) {
                    if (this.app.platformWrapper.getSupportsRestart()) {
                        ...
                        this.dialogs.showInfo(
                            shapezAPI.translations.dialogs.restartRequired.title,
                            shapezAPI.translations.dialogs.restartRequired.text,
                            ["ok:good"]
                        );
                    }
                }
        ...
        if (games.length > 0) {
            ...
            for (let i = 0; i < games.length; ++i) {
                ...
                makeDiv(
                    ...
                    ["level"],
                    games[i].level
                        ? shapezAPI.translations.mainMenu.savegameLevel.replace("<x>", "" + games[i].level)
                        : shapezAPI.translations.mainMenu.savegameLevelUnknown
                );
                ...
                const name = makeDiv(
                    ...
                    ["name"],
                    "<span>" +
                        (games[i].name ? games[i].name : shapezAPI.translations.mainMenu.savegameUnnamed) +
                        "</span>"
                );
                ...
                this.app.adProvider.showVideoAd().then(() => {
                    ...
                    savegame
                        ...
                        .catch(err => {
                            this.dialogs.showWarning(
                                shapezAPI.translations.dialogs.gameLoadFailure.title,
                                shapezAPI.translations.dialogs.gameLoadFailure.text + "<br><br>" + err
                            );
                        });
                });
            });
    ...
    deleteGame(game) {
        ...
        const signals = this.dialogs.showWarning(
            shapezAPI.translations.dialogs.confirmSavegameDelete.title,
            shapezAPI.translations.dialogs.confirmSavegameDelete.text
                .replace("<savegameName>", game.name || shapezAPI.translations.mainMenu.savegameUnnamed)
        ...
        signals.delete.add(() => {
            this.app.savegameMgr.deleteSavegame(game).then(
                ...
                err => {
                    this.dialogs.showWarning(
                        shapezAPI.translations.dialogs.savegameDeletionError.title,
                        shapezAPI.translations.dialogs.savegameDeletionError.text + "<br><br>" + err
                    );
                }
            );
        });
    ...
    showSavegameSlotLimit() {
        const { getStandalone } = this.dialogs.showWarning(
            shapezAPI.translations.dialogs.oneSavegameLimit.title,
            shapezAPI.translations.dialogs.oneSavegameLimit.desc,
                        ...
                        var onMessage = data => {
                            ...
                            if (
                                ...
                                for (let i = 0; i < shapezAPI.modOrder.length; i++) {
                                    ...
                                    if (!gameDataJson.mods.includes(modId))
                                        return canceled(
                                            shapezAPI.translations.multiplayer.notSameMods.title,
                                            shapezAPI.translations.multiplayer.notSameMods.desc
                                        );
                                }
                                for (let i = 0; i < gameDataJson.mods.length; i++) {
                                    const modId = gameDataJson.mods[i];
                                    if (!shapezAPI.modOrder.includes(modId))
                                        return canceled(
                                            shapezAPI.translations.multiplayer.notSameMods.title,
                                            shapezAPI.translations.multiplayer.notSameMods.desc
                                        );
