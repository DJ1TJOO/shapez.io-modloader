...
registerMod({
      ...
      buildings: {
        shape_combiner: {
          default: {
            name: "Shape Combiner",
            description:
              "Merges two regular one layer shapes into one combined shape.",
          },
        },
        belt_crossing: {
          default: {
            name: "Belt Crossing",
            description:
              "Crosses two belts over each other with no interaction between them.",
          },
          corner: {
            name: "Corner Crossing",
            description:
              "Crosses two corner belts over each other with no interaction between them.",
          },
          switcher: {
            name: "Line Crossing",
        ...
        storage: {
          mini: {
            name: "Storage (Mini)",
            description:
              "Stores excess items, up to a given capacity. Prioritizes the top output and can be used as an overflow gate.",
          },
        },
        miner: {
          deep: {
            name: "Extractor (Deep)",
            description:
              "Place over a shape or color to extract it. Extracts more than a normal extractor.",
          },
        },
        ...
        stacker: {
          smart: {
            name: "Smart Stacker",
            description:
              "Combines its inputs, stacking the 3 bottom inputs onto the primary left input.",
          },
        },
        cutter: {
          laser: {
            name: "Smart Cutter",
            description:
              "Cuts a shape, removing the corners indicated on the wires layer.",
          },
        },
        balancer: {
          "splitter-triple": {
        ...
        underground_belt: {
          smart: {
            name: "Smart Tunnel",
            description:
              "Allows you to tunnel resources under buildings and belts, and can also have inputs and outputs from the sides.",
          },
        },
        hyperlink: {
          default: {
            name: "Hyperlink",
            description:
              "Transports up to five belts of items at superfast speed, hold and drag to place multiple.",
          },
          ...
          hyperlink_exit: {
            name: "Hyperlink Exit",
            description:
              "Recieves items from a hyperlink and outputs them onto three belts.",
          },
        },
      },
      storyRewards: {
    ...
    if (root.gameMode.constructor.getId() === SIGameMode.getId()) {
      orginalTranslations = {
        dialogs: {
          ...
          blueprintsNotUnlocked: {
            desc: shapezAPI.translations.dialogs.blueprintsNotUnlocked,
          },
        },
        ingame: {
          keybindingsOverlay: {
            pasteLastBlueprint:
              shapezAPI.translations.ingame.keybindingsOverlay
                .pasteLastBlueprint,
          },
          notifications: {
            researchComplete:
              shapezAPI.translations.ingame.notifications.researchComplete,
          },
          shop: {
        ...
        keybindings: {
          mappings: {
            pasteLastBlueprint:
              shapezAPI.translations.keybindings.mappings.pasteLastBlueprint,
          },
        },
        storyRewards: {
          reward_blueprints: {
      ...
      matchOverwriteRecursive(shapezAPI.translations, {
        dialogs: {
          ...
          blueprintsNotUnlocked: {
            desc: "Complete level 20 to unlock Bugprints!",
          },
        },
        ingame: {
          keybindingsOverlay: {
    ...
    if (root.gameMode.constructor.getId() === SIGameMode.getId()) {
      ...
      root.hubGoals.computeNextGoal = function () {
        ...
        const required = Math.min(
          200,
          Math.floor(4 + (this.level - 40) * 0.25)
        );
        this.currentGoal = {
          ...
          reward:
            this.level % 5 == 0
              ? enumHubGoalRewards.reward_research_level
      ...
      root.hubGoals.canUnlockUpgrade = function (upgradeId) {
        ...
        if (
          ...
          currentLevel >= 6 + (this.level - 40) / 5
        ) {
          // Max level
          return false;
        }
        ...
        for (let i = 0; i < tierData.required.length; ++i) {
          ...
          if (
            (this.storedShapes[requirement.shape] || 0) < requirement.amount
          ) {
            return false;
          }
        }
  ...
  gameBeforeFirstUpdate: (root) => {
    ...
    if (root.gameMode.constructor.getId() === SIGameMode.getId()) {
      var changeIcon = setInterval(function () {
        const shop = document
          .getElementById("ingame_HUD_GameMenu")
  ...
  main: (config) => {
    ...
    shapezAPI.registerBuilding(
      MetaShapeCombinerBuilding,
      "**{icons_building_icon_shape_combiner}**",
      "M"
    );
    shapezAPI.toolbars.buildings.secondaryBuildings.push(
      MetaShapeCombinerBuilding
    );

    shapezAPI.ingame.components[
      BeltCrossingComponent.getId()
    ] = BeltCrossingComponent;
    shapezAPI.ingame.systems.push(BeltCrossingSystem);
    shapezAPI.registerBuilding(
      MetaBeltCrossingBuilding,
      "**{icons_building_icon_belt_crossing}**",
      "K"
    );
    shapezAPI.toolbars.buildings.primaryBuildings.splice(
      shapezAPI.toolbars.buildings.primaryBuildings.indexOf(
        shapezAPI.ingame.buildings.underground_belt
      ) + 1,
      0,
      MetaBeltCrossingBuilding
    );
    ...
    shapezAPI.ingame.components[
      HyperlinkAcceptorComponent.getId()
    ] = HyperlinkAcceptorComponent;
    shapezAPI.ingame.components[
      HyperlinkComponent.getId()
    ] = HyperlinkComponent;
    shapezAPI.ingame.components[
      HyperlinkEjectorComponent.getId()
    ] = HyperlinkEjectorComponent;
    ...
    shapezAPI.registerBuilding(
      ...
      "**{icons_building_icon_hyperlink}**",
      "H"
    );
    shapezAPI.toolbars.buildings.secondaryBuildings.unshift(
      MetaHyperlinkBuilding
    );
    ...
    shapezAPI.ingame.components[
      SmartBalancerComponent.getId()
    ] = SmartBalancerComponent;
    shapezAPI.ingame.systems.push(SmartBalancerSystem);
    addBalancerVariants();

    shapezAPI.ingame.buildings.display.avaibleVariants[
      shapezAPI.exports.defaultBuildingVariant
    ] = (root) =>
    ...
    const typed = (x) => x;
    shapezAPI.exports.enumHubGoalRewardsToContentUnlocked[
      enumHubGoalRewards.reward_belt_crossing
    ] = typed([
      [MetaBeltCrossingBuilding, shapezAPI.exports.defaultBuildingVariant],
    ]);
    shapezAPI.exports.enumHubGoalRewardsToContentUnlocked[
      enumHubGoalRewards.reward_shape_combiner
    ] = typed([
      [MetaShapeCombinerBuilding, shapezAPI.exports.defaultBuildingVariant],
    ]);
    shapezAPI.exports.enumHubGoalRewardsToContentUnlocked[
      enumHubGoalRewards.reward_deep_miner
    ] = typed([
      [
        shapezAPI.ingame.buildings.miner,
        shapezAPI.ingame.buildings.miner.variants.deep,
      ],
    ]);
    shapezAPI.exports.enumHubGoalRewardsToContentUnlocked[
      enumHubGoalRewards.reward_smart_stacker
    ] = typed([
      [
        shapezAPI.ingame.buildings.stacker,
        shapezAPI.ingame.buildings.stacker.variants.smart,
      ],
    ]);
    shapezAPI.exports.enumHubGoalRewardsToContentUnlocked[
      enumHubGoalRewards.no_reward_upgrades
    ] = null;
    shapezAPI.exports.enumHubGoalRewardsToContentUnlocked[
      enumHubGoalRewards.reward_research_level
    ] = null;
    shapezAPI.exports.enumHubGoalRewardsToContentUnlocked[
      enumHubGoalRewards.reward_hyperlink
    ] = typed([
    ...
    shapezAPI.exports.enumHubGoalRewardsToContentUnlocked[
      ...
      [
        shapezAPI.ingame.buildings.underground_belt,
        shapezAPI.ingame.buildings.underground_belt.variants.smart,
      ],
    ]);
    shapezAPI.exports.enumHubGoalRewardsToContentUnlocked[
      enumHubGoalRewards.reward_smart_cutter
    ] = typed([
      [
        shapezAPI.ingame.buildings.cutter,
        shapezAPI.ingame.buildings.cutter.variants.laser,
      ],
    ]);
    shapezAPI.exports.enumHubGoalRewardsToContentUnlocked[
      ...
      [
        shapezAPI.ingame.buildings.balancer,
        shapezAPI.ingame.buildings.balancer.variants.splitterTriple,
      ],
    ]);
    shapezAPI.exports.enumHubGoalRewardsToContentUnlocked[
      ...
      [
        shapezAPI.ingame.buildings.balancer,
        shapezAPI.ingame.buildings.balancer.variants.mergerTriple,
      ],
    ]);
...
function updateSmartUndergroundBeltVariant(entity) {
  ...
  const metaUndergroundBelt = gMetaBuildingRegistry.findByClass(
  ...
  for (let x = affectedArea.x; x < affectedArea.right(); ++x) {
    for (let y = affectedArea.y; y < affectedArea.bottom(); ++y) {
      ...
      for (let i = 0; i < targetEntities.length; ++i) {
        ...
        const targetUndergroundBeltComp =
          targetEntity.components.UndergroundBelt;
        const targetStaticComp = targetEntity.components.StaticMapEntity;

        if (
          !targetUndergroundBeltComp ||
          !(targetUndergroundBeltComp.tier == 2)
        ) {
          // Not a smart tunnel
          continue;
        }
        ...
        metaUndergroundBelt.updateVariants(
          targetEntity,
          rotationVariant,
          MetaUndergroundBeltBuilding.variants.smart
        );
