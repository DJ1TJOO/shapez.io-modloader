...
export const addMinerVariant = () => {
  ...
  miner.avaibleVariants[name] = (root) =>
    root.hubGoals.isRewardUnlocked(enumHubGoalRewards.reward_deep_miner);
  miner.overlayMatrices[name] = () =>
  ...
  miner.setupEntityComponents.push((entity) => {
    if (
      getBuildingDataFromCode(entity.components.StaticMapEntity.code)
        .variant !== name
    )
      return;
    if (entity.components.Miner)
      entity.removeComponent(shapezAPI.exports.MinerComponent);
    if (!entity.components.MinerDeep)
      entity.addComponent(new MinerDeepComponent());
  });
  ...
  miner.additionalStatistics[name] = (root) => {
    ...
    return [
      [
        ...
        shapezAPI.exports.formatItemsPerSecond(speed),
      ],
    ];
  };
};
